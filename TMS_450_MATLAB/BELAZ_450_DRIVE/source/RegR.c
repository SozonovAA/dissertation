...
//попытка отключения перемодуляции

	if(UUdR>11000) UUdR=11000;

	UUqRMAX = GetCatByHypNLeg(UUdR,11500);
...
//попытка убрать перемодуляцию
MinMaxLimitFloat(-11000,11000,&SIdR);
		...
		//попытка убрать перемодуляцию
		MinMaxLimitFloat(-11000,11000,&SIqR);
