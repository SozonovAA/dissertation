...
void RegL(){
...
#endif
	{
		...
		//PARK
		IAlphaL=(2*IaL-IbL-IcL)/3; 
		IBetaL=(IbL-IcL)*(1/sqrt(3)); 
		T0=(IaL+IbL+IcL)/3;

		//Klarc
		co = cos(fThetaL); 
		si = sin(fThetaL); 
		IdL = co*IAlphaL + si*IBetaL; 
		IqL = co*IBetaL - si*IAlphaL; 
...
GetMIN(UUAL-HALF_PWM_HEIGHT,GetMIN(UUBL-HALF_PWM_HEIGHT,UUCL-HALF_PWM_HEIGHT)))/2;
		...
		/*//PARK
		X_=(2*UUAL-UUBL-UUCL)/3; 
		Y_=(UUBL-UUCL)*(1/sqrt(3)); 
		T0=(Ta+Tb+Tc)/3;
